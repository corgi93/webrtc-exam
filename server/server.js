...
*/
function forwardMessageToOtherPerson(sender, message) {
    const receiver = findUserByName(message.otherPerson);
    if (!receiver) {
        // 받는 사람이 없으면 아무것도 리턴하지 x 
        return;
    }
    const json = JSON.stringify(Object.assign(Object.assign({}, message), { otherPerson: sender.name }));
...
function handleSocketConnection(socket) {
    socket.addEventListener('message', (event) => {
        /*
            들어올 메세지들은 버퍼 문자열이므로
            그 버퍼 문자열들을 JSON.parst()로 Obejct로 convert해야함
